...
There are two main categories of robots: mobile robots (e.g. autonomous ground/areal/underwater vehicles) and manipulators (e.g. industrial robot arms, collaborative robot arms). In this course, we focus on manipulators and more specifically on industrial robot arms, which are the most common type of robot in manufacturing.

- a bunch of motors, encoders connected by some metal; and also a controller that makes everything move in a coordinated way

- a robot arm is basically a "motion generator" - a fancy, programmable, multi-axis positioning system. It can be used to move things around, but it can also be used to hold tools, sensors, or end-effectors that perform some operation on the workpiece.

- a robot cell is a robot arm plus some way of presenting parts to the robot, and some way of taking them away after the robot has done its work. The cell may also include fixtures, grippers, sensors, safety equipment, and a controller that coordinates everything.
