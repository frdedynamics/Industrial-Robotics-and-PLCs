...
Treat a displayed robot workspace as a rough, position-based reach envelope for
initial layout design. It does not show that every tool orientation is possible
at every position. The [difference between reachable and dexterous
workspace](kinematics.qmd#sec-workspace-types) explains this limitation. A task
is feasible only if the selected tool and workpiece can reach every required
pose with acceptable joint values, robot configuration, clearance, and
approach path. VC provides workspace profiles, joint-limit feedback,
configuration tools, and collision detectors to support these checks.
