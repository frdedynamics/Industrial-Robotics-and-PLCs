...
@fig-pally-dimensions shows the same parametric idea on the UR pendant: the user
enters box dimensions, and the palletizing program uses those values instead of
being rewritten for each new packing task.

![Pally URCap interface for entering box dimensions for a palletizing task. These values become parameters used by the palletizing program. Image source: [Pally URCap User Manual, version 2.12](https://www.rocketfarm.no/wp-content/uploads/2022/08/User-Manual-Pally-URCap-2.12.pdf).](img/pally_dimensions.png){#fig-pally-dimensions fig-alt="Pally URCap screen on a Universal Robots teach pendant for entering box dimensions"}
...
| Offline programming / simulation | Complex cells, expensive downtime, [virtual commissioning](simulation.qmd#sec-virtual-commissioning) | Test before hardware, minimized downtime, collision checks, cycle-time estimates | Needs accurate models and calibration |
