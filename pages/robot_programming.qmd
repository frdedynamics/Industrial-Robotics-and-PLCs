...
poses, motion commands, tool actions, and logic.
...
important poses or along a path.

```{=html}
<figure class="video-embed" id="fig-hand-guiding-video">
  <iframe
  ...
  <figcaption>Example of hand-guided teaching from 0:56 to 1:29. The full video is available on <a href="https://www.youtube.com/watch?v=hCfn0mzHLyM">YouTube</a>.</figcaption>
</figure>
```
...
continue producing while the next program version is prepared.
...
is then translated into machine or robot instructions by a postprocessor.

![CAM-style programming converts part geometry and process settings into a toolpath, then into robot-specific commands.](img/cam_toolpath_schematic.svg){#fig-cam-toolpath fig-alt="Schematic CAM workflow from CAD model to toolpath to robot program"}
...
The two seam-tracking examples below show why the path may need to be corrected
while the robot is moving. In the optical example, an external sensor measures
where the seam is. In the second example, the correction is based on welding
process data instead of an external optical seam sensor.

```{=html}
<div class="video-grid">
  <figure class="video-embed" id="fig-optical-seam-tracking">
    <iframe
    ...
    <figcaption>Optical seam tracking example. Source: <a href="https://www.youtube.com/watch?v=pZlhQDYey08">YouTube</a>.</figcaption>
  </figure>
  <figure class="video-embed" id="fig-weld-data-seam-tracking">
    <iframe
    ...
    <figcaption>Seam tracking example using welding process data instead of a separate optical seam sensor. Source: <a href="https://www.youtube.com/watch?v=SO0GmVYE_4U">YouTube</a>.</figcaption>
  </figure>
</div>
```
...
Robot learning tries to move beyond fixed instruction sequences. Instead of
programming every detail, the robot learns a behavior from data, interaction, or
demonstrations.

A common industrially relevant idea is **learning from demonstration** or
...
uses those examples to reproduce or generalize the behavior [@Argall.2009].
...
| Offline programming / simulation | Complex cells, expensive downtime, virtual commissioning | Test before hardware, collision checks, cycle-time estimates | Needs accurate models and calibration |
...
A useful rule of thumb is to separate **where the path comes from** from **how
the cell is controlled**. A path can be taught online, generated in CAM, or
computed from a camera measurement. But the full cell still needs frames, robot
motions, I/O, safety behavior, error handling, and PLC coordination.

# Practical workflow for this course

For the simulation and programming parts of the course, a practical workflow is:

1. Define the task in plain language: what must be moved, processed, or
   inspected?
2. Identify the relevant frames: robot base, tool/TCP, workpiece, fixture,
   camera, or conveyor.
3. Decide which targets are fixed and which targets are computed from product data or
   sensors.
4. Choose movement types: joint moves for repositioning, linear or circular
   moves for process paths.
5. Build the sequence: wait conditions, robot motions, tool actions, and PLC
   handshakes.
6. Test reachability, collisions, singularities, and cycle time in simulation.
7. Generate or export the robot program if the workflow supports it.
8. Plan what must be checked on the real cell: calibration, payload, speeds,
   safety zones, I/O signals, and recovery behavior.

This mirrors how industrial robot work is usually done. The exact software and
vendor language may change, but the engineering questions are stable.

# Common mistakes

Typical beginner mistakes are:

- storing targets in the wrong frame,
- teaching points that work only for one exact fixture position,
- using linear motion where a joint move would be safer and simpler,
- forgetting approach and retreat poses,
- testing only the happy path and not missing-part or failed-grip cases,
- trusting an offline model without checking calibration,
- treating the robot program separately from the PLC and safety system.

Good robot programming is therefore partly about syntax, but mostly about
systematic thinking. The robot should not only move correctly when everything is
perfect. It should also behave predictably when the cell is waiting, blocked,
empty, misaligned, or recovering from an error.

# Exercises

1. Choose one task, for example palletizing, welding, machine tending, or glue
   dispensing. Which programming approach would you choose first, and why?
2. For the same task, list which frames would be needed in the program.
3. Identify one part of the task that could be taught online and one part that
   would be better generated offline or parametrically.
4. In an offline simulation, what information must match the real cell before
   you would trust the program?
5. Consider a camera-guided pick task. What data must pass from the camera to
   the robot, and what role does the PLC play?
6. Open ISO 10218-1:2025 and compare the terms **teach**, **teach pendant**,
...
7. Use ISO 10218-1:2025, clause 5.10.2, to list three safety functions or design
